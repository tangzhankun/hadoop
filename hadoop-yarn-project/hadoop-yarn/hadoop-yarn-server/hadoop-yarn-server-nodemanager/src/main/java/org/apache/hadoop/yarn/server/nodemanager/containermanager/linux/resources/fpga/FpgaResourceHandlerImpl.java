  ...
  public List<PrivilegedOperation> preStart(Container container) throws ResourceHandlerException {
    ...
    try {
      ...
      FpgaResourceAllocator.FpgaAllocation allocation = allocator.assignFpga(
          ...
          container.getContainerId(), getRequestedIPID(container));
      LOG.info("FpgaAllocation:" + allocation);
      //update cgroup device param, we use GPU interface for test
      //TODO: change to FPGA native module interfaces
      PrivilegedOperation privilegedOperation = new PrivilegedOperation(PrivilegedOperation.OperationType.GPU,
