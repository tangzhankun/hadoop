...
#define DEFAULT_ALTERA_MAJOR_NUMBER 195
...
static int internal_handle_fpga_request(
  ...
  // Get major device number from cfg, if not set, major number of (Altera)
  ...
  if (!major_number_str || 0 == major_number_str[0]) {
    // Default major number of Altera devices
    major_device_number = DEFAULT_ALTERA_MAJOR_NUMBER;
  } else {
